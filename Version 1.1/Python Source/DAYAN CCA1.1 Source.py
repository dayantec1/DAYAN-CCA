...
class CiscoSwitchManager:
    def __init__(self, root):
        self.root = root
        self.root.title("Cisco Configuration Assistant | DAYAN TEC. v1.1")
    ...
    def create_management_page(self):
        ...
        frame5.pack(pady=5)
        tk.Button(frame5, text="NTP تنظیمات", command=self.ntp_config, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)
        tk.Button(frame5, text="Interface به یک IP اختصاص", command=self.IP_assign, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)
        tk.Button(frame5, text="VLAN ساخت یک", command=self.create_VLAN, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)
        tk.Button(frame5, text="نمایش جدول مسیریابی", command=self.show_route_table, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)
        tk.Button(frame5, text="Interface مشاهد وضعیت یک", command=self.show_interface, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)

        frame6=tk.Frame(self.root)
        frame6.pack(pady=5)
        tk.Button(frame6, text="VLAN حذف یک", command=self.delete_VLAN, bg="orange", fg="black", width=30, height=2).pack(side=tk.LEFT,padx=5)

        frame4=tk.Frame(self.root)
        frame4.pack(pady=(300,10))
        ...
        if confirm:
            ...
            if cmd:
                ...
                if "[OK]" in output :
                    ...
                    messagebox.showinfo("ذخیره اطلاعات", ".خطایی رخ داده ، لطفا مجددا تلاش کنید")
    ...
    def IP_assign(self):
        ...
        if "C2960" in output :
            self.show_output(output)
            messagebox.showinfo("خظا", ".دستگاه شما از این ویژگی پشتیبانی نمیکند")
        else :
            values=[
               "\t(GigabitEthernet0/1 مثلاً)Interface نام :",
               "\tIP آدرس:",
            ...
            user_inputs= easygui.multenterbox("لطفا مقادیر زیر را وارد کنید ","ورود اطلاعات",values)
            interface , IP_address , mask = user_inputs
            if interface and IP_address and mask :
                commands = [
                    f'configure terminal',
                    f'interface {interface}',
                    f'no switchport',
                ...
                if {IP_address} in output :
                    ...
                    messagebox.showinfo("ذخیره اطلاعات", ".خظایی رخ داده ، لطفا مجددا تلاش کنید")
    ...
    def ntp_config(self):
        ...
        user_inputs= easygui.multenterbox("لطفا مقادیر زیر را وارد کنید ","ورود اطلاعات",values)
        name_server , ntp_peer = user_inputs
        if name_server and ntp_peer :
            commands = [
                f'configure terminal',
                f'ip name-server {name_server}',
                f'ntp peer {ntp_peer}',
                f'clock timezone Tehran 3 30',
                f'end',
            ]
            output = self.connection.send_config_set(commands)
            output = self.connection.send_config_set("show ntp config")
            self.show_output(output)
            if {ntp_peer} in output :
                messagebox.showinfo("ذخیره اطلاعات", ".تنظیمات مورد نظر با موفقیت ذخیره شد")
            else :
                messagebox.showinfo("ذخیره اطلاعات", ".خطایی رخ داده ، لطفا مجددا تلاش کنید")
    ...
    def create_VLAN(self):
        values=[
            "\t(Users,Office,... مثلاً)VLAN نام :",
            "\tVLAN شماره :"
        ]
        user_inputs= easygui.multenterbox("لطفا مقادیر زیر را وارد کنید ","ورود اطلاعات",values)
        vlan_name , vlan_id = user_inputs
        if vlan_name and vlan_id:
            ...
            output = self.connection.send_config_set(commands)
            output = self.connection.send_config_set("do show vlan brief")
            self.show_output(output)
            if {vlan_id} or {vlan_name} in output :
                messagebox.showinfo("ذخیره اطلاعات", ".تنظیمات مورد نظر با موفقیت ذخیره شد")
            else :
                messagebox.showinfo("ذخیره اطلاعات", ".خطایی رخ داده ، لطفا مجددا تلاش کنید")
    ...
    def delete_VLAN(self):
        vlan_id = simpledialog.askstring("ورود اطلاعات", "\t\t\t :VLAN شماره")
        if vlan_id:
            commands = [
                f'configure terminal',
                f'no vlan {vlan_id}',
            ...
            output = self.connection.send_config_set("do show vlan brief")
            self.show_output(output)
            messagebox.showinfo("ذخیره اطلاعات", ".ها را بررسی کنیدVLAN انجام شد، لطفا مجددا وضعیت ")
    ...
    def show_route_table(self):
        ...
        if "C2960" in output :
            ...
            output = self.connection.send_command("show ip route")
            self.show_output(output)
    ...
    def show_help(self):
        help_message = """این نرم افزار توسط شرکت فنی و مهندسی دایان تکنولوژی جهت سهولت در مدیریت سوئیچ های شرکت سیسکو طراحی و توسعه داده شده است ، جهت کسب اطلاعات بیشتر با ما در ارتباط باشید ،نسخه 1.1www.dayantec.com | 02196625007 | info@dayantec.com"""
